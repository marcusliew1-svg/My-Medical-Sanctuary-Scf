...
5. Run `database/provision/002_mms_commercial_runtime_role_verify.sql` and confirm all `*_should_be_false` privilege checks are false, RLS is enabled on all approved commercial tables, the runtime policy count matches the approved table count, and `submit_partner_application` is executable by the runtime role.
...
## Partner application controls

Migration `0006_mms_partner_application_submission.sql` is mandatory before Partner-facing application submission is enabled. It requires the authenticated Partner to resolve to an Active, selling-enabled, CRM-enabled Partner with current certification. The lead must be owned by that Partner and already be Qualified. Submission is atomic and idempotent, a lead cannot have two simultaneous non-terminal applications, and the lead/application workflow events are written in the same database transaction.

The Partner Hub application form accepts only a Lead ID and one controlled commercial membership code: Ascend, Evolve, Eterna or Pinnacle. It does not accept diagnosis, symptoms, treatment selection, test results or other clinical information.
...
11. Create or transition a QA lead to Qualified, submit an application through the Partner Hub, and confirm the application is attributed only to the authenticated Partner.
12. Retry the same application operation using the same idempotency key and confirm the existing application is returned without a duplicate application or duplicate workflow event.
13. Attempt to submit another open application for the same lead and confirm it is rejected.
14. Attempt submission while the Partner is Suspended, certification-expired, selling-disabled or CRM-disabled and confirm it is rejected.
...
- Partner application submission is Partner-scoped, certification-gated and idempotent;
- duplicate open applications for one lead are rejected;
...
- application submission remains CSRF-protected and accepts commercial fields only;
