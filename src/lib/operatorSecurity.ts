...
export function operatorRequestOriginAllowed(request: NextRequest): boolean {
...
export async function requireOperatorMutation(
  ...
  if (!operatorRequestOriginAllowed(request)) return { status: "forbidden", reason: "Operator mutation origin is not permitted." };
