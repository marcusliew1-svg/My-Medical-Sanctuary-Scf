...
import { getTreatmentEducation, treatmentEducation } from "@/data/treatmentEducation";
...
export function generateStaticParams() {
  return treatmentEducation.map((item) => ({ slug: item.slug }));
}

export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  const item = getTreatmentEducation(params.slug);
  if (!item) return {};
  return {
    ...
    <main>
      <section className="relative isolate overflow-hidden bg-[#062e29] px-4 pb-16 pt-28 text-ivory md:pt-36">
        ...
        <div className="mx-auto max-w-5xl">
          <Link href="/treatments" className="text-xs font-bold uppercase tracking-[.18em] text-[#d7c9a7]">← Treatments & wellness guide</Link>
      ...
      <section className="bg-ivory px-4 py-16"><div className="mx-auto max-w-5xl"><p className="text-xs font-bold uppercase tracking-[.16em] text-deep-green">What a responsible pathway looks like</p><h2 className="mt-3 font-serif text-4xl text-navy md:text-5xl">From interest to an informed decision.</h2><div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{item.typicalJourney.map((step,index)=><article key={step} className="rounded-[1.5rem] border border-stone-200 bg-white p-5 shadow-soft"><span className="grid size-10 place-items-center rounded-full bg-deep-green text-xs font-bold text-white">0{index+1}</span><p className="mt-4 text-sm leading-6 text-navy">{step}</p></article>)}</div></div></section>

      <section className="bg-warm-white px-4 py-16"><div className="mx-auto grid max-w-5xl gap-7 lg:grid-cols-[.85fr_1.15fr]"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-deep-green">Questions to take to your doctor</p><h2 className="mt-3 font-serif text-4xl text-navy">Research should improve the consultation.</h2><p className="mt-4 leading-7 text-warm-gray">Use these as conversation starters rather than a checklist for self-treatment.</p></div><div className="grid gap-3">{item.questions.map((q,index)=><div key={q} className="flex gap-4 rounded-2xl border border-stone-200 bg-white p-5"><span className="font-serif text-2xl text-deep-green">0{index+1}</span><p className="pt-1 text-sm leading-6 text-navy">{q}</p></div>)}</div></div></section>

      {item.relatedConcerns.length ? <section className="bg-ivory px-4 py-14"><div className="mx-auto max-w-5xl"><p className="text-xs font-bold uppercase tracking-[.16em] text-deep-green">Related health concerns</p><div className="mt-5 flex flex-wrap gap-3">{item.relatedConcerns.map(link=><Link key={link.href} href={link.href} className="rounded-full border border-deep-green/20 bg-white px-5 py-3 text-sm font-bold text-deep-green transition hover:border-deep-green">{link.label} →</Link>)}</div></div></section> : null}
