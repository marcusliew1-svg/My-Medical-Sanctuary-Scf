...
export async function GET(request: NextRequest, context: { params: Promise<{ transactionId: string }> }) {
  ...
  const transactionId = transactionIdParam.trim();
  ...
  try {
    ...
    const canMutate = operator.claims.roles.includes("admin") || operator.claims.roles.includes("finance");
    return NextResponse.json({ status: "ok", detail, capabilities: { canMutate } }, { headers: { "Cache-Control": "no-store, max-age=0", Pragma: "no-cache" } });
  } catch {
