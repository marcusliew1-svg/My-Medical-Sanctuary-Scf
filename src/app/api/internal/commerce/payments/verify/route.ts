...
export async function POST(request: NextRequest) {
  if (!financeApiConfigured()) {
    return NextResponse.json({ status: "unavailable", message: "Finance verification controls are not configured." }, { status: 503 });
  }
  ...
  if (
    ...
    !verifiedAt ||
    !/^[A-Z]{3}$/.test(currency) ||
    !verificationSources.has(source) ||
    !sourceReference ||
    !Number.isInteger(clearedAmountMinorUnits)
  ) {
  ...
  if (!recordResult.value.payment) {
    return NextResponse.json({ status: "payment_missing", message: "The application has no payment record to verify." }, { status: 409 });
  }
  if (recordResult.value.payment.paymentId !== paymentId) {
    return NextResponse.json({ status: "payment_conflict", message: "Payment ID does not match the application payment record." }, { status: 409 });
  }
  ...
  try {
    const verified = financeVerifyPayment({
      application: recordResult.value.application,
      payment: recordResult.value.payment,
      evidence,
    });
    const saveResult = await store.savePaymentVerification({
    ...
    return NextResponse.json({
      status: "verified",
      applicationId,
      applicationStage: verified.application.stage,
      paymentId,
      paymentStage: verified.payment.stage,
      clearedAt: verified.payment.clearedAt,
    });
  } catch (error) {
    return NextResponse.json(
      { status: "invalid_transition", message: error instanceof Error ? error.message : "Payment verification could not be applied." },
      { status: 409 },
    );
  }
}
