...
export async function POST(request: NextRequest) {
  ...
  const submittedAt = cleanString(body.submittedAt, 80);
  const recordedBy = cleanString(body.recordedBy, 160);
  ...
  if (
    ...
    (amountMinorUnits as number) <= 0 ||
    !/^[A-Za-z0-9._:-]{16,120}$/.test(idempotencyKey)
  ) {
    return NextResponse.json(
      { status: "invalid", message: "A valid application, payment reference, amount, currency, timestamp, recorder and Idempotency-Key are required." },
      { status: 400 },
    );
  }
  ...
  if (result.status === "conflict") {
    return NextResponse.json({ status: "payment_conflict", message: result.reason }, { status: 409 });
  }

  const payment = result.value.record.payment;
  if (!payment) {
    return NextResponse.json({ status: "conflict", message: "Recorded payment could not be reloaded." }, { status: 409 });
  }
