...
const ALLOWED_FIELDS = new Set(["transactionId", "payoutBatchId", "payoutReference", "paidBy"]);
...
export async function POST(request: NextRequest) {
  ...
  if (Object.keys(body).some((field) => !ALLOWED_FIELDS.has(field))) {
    return NextResponse.json({ status: "invalid", message: "Unexpected commission payout fields were supplied." }, { status: 400 });
  }
  ...
  if (!/^[A-Za-z0-9_-]{8,100}$/.test(transactionId) || !payoutBatchId || !payoutReference || !paidBy) {
    return NextResponse.json({ status: "invalid", message: "transactionId, payoutBatchId, payoutReference and paidBy are required." }, { status: 400 });
  }
  ...
  if (existing.status === "Paid") {
    if (existing.payoutBatchId === payoutBatchId && existing.payoutReference === payoutReference && existing.paidBy === paidBy && existing.paidAt) {
      return NextResponse.json({
        status: "already_paid",
        transactionId,
        approvedCommissionMinorUnits: existing.approvedCommissionMinorUnits,
  ...
  try {
    const transition = markCommissionPaid({ transaction: existing, payoutBatchId, payoutReference, paidBy, paidAt: new Date().toISOString() });
    ...
    return NextResponse.json({
      ...
      payoutBatchId: saved.value.transaction.payoutBatchId,
      payoutReference: saved.value.transaction.payoutReference,
      paidAt: saved.value.transaction.paidAt,
    });
  } catch (error) {
