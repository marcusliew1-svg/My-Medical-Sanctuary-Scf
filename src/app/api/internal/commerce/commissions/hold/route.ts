...
const ALLOWED_FIELDS = new Set(["transactionId", "action", "actor", "reason"]);
...
export async function POST(request: NextRequest) {
  ...
  if (Object.keys(body).some((field) => !ALLOWED_FIELDS.has(field))) {
    return NextResponse.json({ status: "invalid", message: "Unexpected commission hold fields were supplied." }, { status: 400 });
  }
  ...
  if (!/^[A-Za-z0-9_-]{8,100}$/.test(transactionId) || !["hold", "release"].includes(action) || !actor || !reason) {
    return NextResponse.json({ status: "invalid", message: "transactionId, action (hold/release), actor and reason are required." }, { status: 400 });
  }
  ...
  if (action === "hold") {
    if (current.status === "Held") {
      ...
      if (latestHold?.actor === actor && latestHold.reason === reason) {
        return NextResponse.json({ status: "already_held", transactionId, heldAt: latestHold.occurredAt, reason: latestHold.reason });
      }
    ...
    if (current.status !== "Eligible") {
      return NextResponse.json({ status: "invalid_state", message: "Only Eligible commission can be placed on hold." }, { status: 409 });
    }

    const transaction: CommissionTransaction = { ...current, status: "Held", holdReason: reason };
    const event: CommissionLedgerEvent = {
      eventId: `COM-HOLD-${transactionId}-${Date.parse(occurredAt)}`,
      transactionId,
  ...
  const event: CommissionLedgerEvent = {
    eventId: `COM-RELEASE-${transactionId}-${Date.parse(occurredAt)}`,
    transactionId,
    previousStatus: "Held",
    nextStatus: "Eligible",
    actor,
