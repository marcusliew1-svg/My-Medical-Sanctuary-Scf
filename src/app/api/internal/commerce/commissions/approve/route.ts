...
const ALLOWED_FIELDS = new Set(["transactionId", "approvedBy"]);
...
export async function POST(request: NextRequest) {
  ...
  if (Object.keys(body).some((field) => !ALLOWED_FIELDS.has(field))) {
    return NextResponse.json({ status: "invalid", message: "Unexpected commission approval fields were supplied." }, { status: 400 });
  }
  ...
  if (existing.status === "Approved" || existing.status === "Paid") {
    if (existing.approvedBy === approvedBy && existing.approvedAt) {
  ...
  try {
    const transition = approveCommissionTransaction({ transaction: existing, approvedBy, approvedAt: new Date().toISOString() });
    ...
    return NextResponse.json({
      ...
      approvedAt: saved.value.transaction.approvedAt,
    });
  } catch (error) {
