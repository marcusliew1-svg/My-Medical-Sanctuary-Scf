...
type DetailResponse = { status: string; message?: string; detail?: { transaction: Record<string, unknown>; events: Array<Record<string, unknown>> } };
...
export default function CommissionDetailClient({ transactionId }: { transactionId: string }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setDetail(body.detail); setMessage("");
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-base font-semibold">Finance controls</h2><p className="mt-1 text-sm text-slate-600">Current status: <span className="font-semibold text-slate-900">{status}</span>. Approval, payout and reversal require recent Finance step-up.</p></div></div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        {status !== "Reversed" ? <button disabled={busy || !reason.trim()} onClick={() => void mutate("/api/internal/commerce/commissions/reverse", { transactionId, reason })} className="rounded-xl border border-red-300 px-4 py-2 text-sm font-semibold text-red-700 disabled:opacity-40">Reverse</button> : null}
      </div>
