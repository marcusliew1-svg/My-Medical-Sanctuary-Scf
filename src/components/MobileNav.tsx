...
export function MobileNav() {
  ...
  return (
    <div className="xl:hidden">
      <button type="button" aria-label={open ? "Close navigation" : "Open navigation"} aria-expanded={open} onClick={() => setOpen((value) => !value)} className="grid size-11 place-items-center rounded-full border border-white/30 bg-white/[0.14] backdrop-blur-md">
      ...
      {open ? (
        <div className="absolute inset-x-2 top-20 max-h-[calc(100vh-6rem)] overflow-y-auto rounded-2xl border border-white/20 bg-navy/98 p-4 text-white shadow-[0_30px_80px_rgba(0,0,0,0.35)] backdrop-blur-xl sm:inset-x-4 sm:p-5">
          <nav aria-label="Mobile navigation" className="grid gap-1">
            {navigation.map((item) => <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className="rounded-md px-3 py-3 text-sm font-semibold text-white/[0.84] transition hover:bg-white/[0.08] hover:text-white">{item.label}</Link>)}
          </nav>
          <div className="mt-4 grid gap-2 border-t border-white/10 pt-4">
            ...
            <Link href="/ling" onClick={() => setOpen(false)} className="rounded-md bg-gold px-3 py-3 text-sm font-semibold text-navy">Start with Ling</Link>
          </div>
        </div>
      ) : null}
    </div>
