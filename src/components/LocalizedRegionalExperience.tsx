...
export type RegionalLocale = "ms" | "zh" | "th";
export type RegionalSection = "ling" | "memberships" | "treatments" | "health-concerns" | "clinics" | "medical-tourism" | "online-doctor" | "contact";
...
const sectionOrder: RegionalSection[] = ["ling", "memberships", "treatments", "health-concerns", "clinics", "medical-tourism", "online-doctor", "contact"];
...
export function isRegionalSection(value: string): value is RegionalSection { return sectionOrder.includes(value as RegionalSection); }

function LocaleNav({ locale }: { locale: RegionalLocale }) {
  const content = copy[locale];
  return <div className="mt-10 flex flex-wrap gap-2">{sectionOrder.map((section) => <Link key={section} href={`/${locale}/${section}`} className="rounded-full border border-deep-green/15 bg-white px-4 py-2 text-xs font-semibold text-deep-green shadow-sm transition hover:border-deep-green/35">{content.nav[section]}</Link>)}</div>;
}

export function LocalizedRegionalHome({ locale }: { locale: RegionalLocale }) {
  const content = copy[locale];
  return <main className="min-h-screen bg-ivory px-4 pb-24 pt-36"><div className="mx-auto max-w-6xl">
