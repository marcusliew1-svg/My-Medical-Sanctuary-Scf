...
do $$ begin
  if not exists (select 1 from mms_commercial.schema_migrations where migration_key='0023_mms_health_intelligence_assisted_ingestion.sql') then
    raise exception '0023 migration ledger entry missing';
  end if;
...
select id,'qa-connector-0023','csv','QA controlled CSV connector',market_id,'approved',
  'qa','qa',now(),'{}'::jsonb
...
do $$ begin
  begin
    ...
    select id,'qa-secret-rejection-0023','api','QA secret rejection',market_id,'approved',
