-- MMS Partner application evidence hardening.
-- Apply only to the dedicated MMS commercial database after migration 0017.
-- Commercial application controls only. Never apply to iPivot or clinical/patient databases.
...
begin
  ...
  where upper(p.partner_code) = upper(trim(p_partner_code))
    ...
    and nullif(trim(coalesce(p.agreement_version,'')), '') is not null
    and p.agreement_accepted_at is not null
    and p.agreement_accepted_at <= p_submitted_at
    and nullif(trim(coalesce(p.compliance_version,'')), '') is not null
    and p.compliance_acknowledged_at is not null
    and p.compliance_acknowledged_at <= p_submitted_at
    and exists (
  ...
  select l.id, l.stage, l.duplicate_status into v_lead_id, v_lead_stage, v_lead_duplicate_status
  ...
  if v_lead_duplicate_status <> 'Clear' then
  ...
  insert into mms_commercial.commercial_workflow_events(
    ...
    'Partner submitted a commercial membership application after duplicate, agreement, compliance and certification evidence checks.', p_submitted_at
  );
...
values (
  ...
  'Application submission now independently requires Clear duplicate status plus Partner agreement, compliance and certification evidence current at submission time.'
)
