-- MMS commission eligibility provenance hardening.
-- Apply only to the dedicated MMS commercial database after migration 0014.
-- Commercial attribution and compliance evidence only. Never apply to iPivot or a clinical/patient database.
...
begin
  ...
  if v_lead.id is null then
    raise exception using errcode='P0001', message='lead_attribution_missing';
  end if;
  if v_lead.duplicate_status <> 'Clear' then
    raise exception using errcode='P0001', message='lead_duplicate_clearance_required';
  end if;
  if v_lead.current_partner_id <> v_app.partner_id then
    raise exception using errcode='P0001', message='lead_partner_attribution_conflict';
  end if;

  select * into v_partner
  from mms_commercial.partners
  where id=v_app.partner_id
  for share;
  ...
  if v_partner.stage <> 'Active'
     ...
     or v_partner.level is null then
    raise exception using errcode='P0001', message='partner_not_commission_eligible';
  end if;
  if nullif(trim(coalesce(v_partner.agreement_version,'')), '') is null
     or v_partner.agreement_accepted_at is null
     or v_partner.agreement_accepted_at > p_checked_at then
    raise exception using errcode='P0001', message='partner_agreement_acceptance_required';
  end if;
  if nullif(trim(coalesce(v_partner.compliance_version,'')), '') is null
     or v_partner.compliance_acknowledged_at is null
     or v_partner.compliance_acknowledged_at > p_checked_at then
    raise exception using errcode='P0001', message='partner_compliance_acknowledgement_required';
  end if;
  if not exists (
  ...
  insert into mms_commercial.commission_events(
    ...
    'Eligibility verified from persisted lead ownership and duplicate clearance, Partner agreement/compliance/certification, cleared payment, active membership and approved commission rule ' || v_rule.version || '.',
    p_checked_at
  );
...
values(
  '0015_mms_commission_eligibility_provenance.sql',
  'Requires Clear lead duplicate status, intact current Partner attribution, agreement acceptance, compliance acknowledgement, current certification and existing Finance/membership controls before commission eligibility.'
)
